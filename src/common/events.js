...
var WeavyEvents = function (rootTarget) {
  ...
  function getEventArguments(contextTarget, eventArguments) {
    ...
    var localEvent = typeof eventArguments[0] === "string" && eventArguments[0];
      ...
      if (args.external) {
        if (typeof args.selector === "string" || WeavyUtils.isPlainObject(args.selector)) {
          if (typeof args.context.one === "function") {
            ...
            console.warn("external .one() target does not support selectors")
          }
        } else {
          if (typeof args.context.one === "function") {
            ...
            console.warn("external target does not have valid event listening");
          }
        }
      }
    } else {
      ...
      if (args.external) {
        if (typeof args.selector === "string" || WeavyUtils.isPlainObject(args.selector)) {
          if (typeof args.context.one === "function") {
            ...
            console.warn("external .on() target does not support selectors")
          }
        } else {
