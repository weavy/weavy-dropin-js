...
var WeavyHistory = function (weavy) {
   ...
   * 
   * @example
   * var weavyState = weavy.history.getStateFromUri("wvy://app-files@wvy-id/dropin/files/123");
   * 
   * @example
   * var weavyUris = [
   *   "wvy://app-files@wvy-id/dropin/files/123",
   *   "wvy://app-posts/dropin/posts/456"
   * ];
   * 
  ...
  weavyHistory.getStateFromPanel = function (panelId) {
    ...
    return {
      ...
      title: panel.node.dataset.title,
      weavyId: weavyId,
      weavyUri: weavyUri,
  ...
  weavyHistory.openState = function (state) {
    if (state) {
      if (state && state.panels && Array.isArray(state.panels)) {
        return weavy.whenReady().then(function () {
          ...
          return Promise.all(state.panels.map(function (panelState) {
            if (!panelState.weavyId && !weavy.options.id || panelState.weavyId === weavy.getId()) {
              if (panelState.panelId.startsWith("overlay:")) {
                weavy.overlays.openState(panelState);
              } else {
  ...
  function setCurrentHistoryState() {
    ...
    weavy.debug("history: set current state", state);
    weavyHistory.setBrowserState(state, "replace");
  }

  weavy.on(window, "popstate", function () {
    var state = weavyHistory.getBrowserState();
    if (state) {
      weavy.debug("history: popstate", state);
      weavyHistory.openState(state);
    }
  });

  weavy.whenInitialized().then(function () {
    var state = weavyHistory.getBrowserState();
    if (state) {
      weavy.debug("history: load state", state);
      weavyHistory.openState(state)
    }

    weavy.whenLoaded().then(weavy.whenTimeout(1)).then(setCurrentHistoryState);

  });
};

export default WeavyHistory;
 ...
 *   statusCode: 200,
 *   weavyId: "wy-41d751e8",
 *   weavyUri: "wvy://app-files@wy-41d751e8/dropin/files/123",
 ...
 * "wvy://app-files@wvy-id/dropin/files/123"
 *
 * @typedef {string} WeavyHistory~weavyUri
 */
