...
var WeavyPanel = function (weavy, _panels, panelsContainer, panelId, url, attributes) {
  ...
  if (attributes.title) {
    panel.node.dataset.title = attributes.title;
  }
  ...
  var _onReady = function (e, ready) {
    ...
    _isReady = true;
    panel.triggerEvent("before:panel-ready", { panelId: panelId });
    ...
    if (typeof ready.title === "string" && ready.title) {
      panel.node.dataset.title = ready.title;
    }
     ...
     * @property {string} panelId - The id of the panel loading.
     */
    panel.triggerEvent("on:panel-ready", { panelId: panelId });

    panel.whenReady.resolve(panel);
    panel.triggerEvent("after:panel-ready", { panelId: panelId });
  };

  var _onUnready = function() {
    panel.triggerEvent("before:panel-unready", { panelId: panelId });
     ...
     */
    panel.triggerEvent("on:panel-unready", { panelId: panelId });

    panel.triggerEvent("after:panel-unready", { panelId: panelId });
    
  }

  weavy.on(WeavyPostal, "ready", { weavyId: weavy.getId(), windowName: panel.frame.name }, _onReady);
  weavy.on(WeavyPostal, "unready", { weavyId: weavy.getId(), windowName: panel.frame.name }, _onUnready);

  var unregisterReady = function () {
    weavy.off(WeavyPostal, "ready", { weavyId: weavy.getId(), windowName: panel.frame.name }, _onReady);
    weavy.off(WeavyPostal, "unready", { weavyId: weavy.getId(), windowName: panel.frame.name }, _onUnready);
  };
  ...
  weavy.on(WeavyPostal, "load", { weavyId: weavy.getId(), windowName: panel.frame.name }, function () {
  ...
  panel.close = function (noHistory, noEvent) {
    ...
    return weavy.whenReady().then(function () {
      ...
      if (panel.isOpen) {
        ...
        closePromises.push(panel.postMessage({ name: 'close' }).catch(() => {
          weavy.warn("Unable to close panel", panel.panelId);
        }));


        // Return timeout promise
        closePromises.push(weavy.whenTimeout(250));
  ...
  panel.load = function (url, data, method, replace, noHistory) {
    return weavy.whenReady().then(function () {
      ...
      if (url) {
        ...
        if (panel.isReady) {
          weavy.log("panel is ready load")
          if (replace) {
            panel.loadingStarted(true);
          }
          // Fully loaded, send using turbolinks
          panel.postMessage({ name: 'turbo-visit', url: url, data: data, method: method, action: "replace" });
        } else {
          weavy.log("panel not ready load")
  ...
  panel.reset = function () {
    return weavy.whenReady().then(function () {
      ...
      if (oldFrame) {
        ...
        try {
          WeavyPostal.unregisterContentWindow(oldFrame.name, weavy.getId());
        } catch (e) {
  ...
  panel.setState = function (state) {
    ...
    if (state.title) {
      panel.node.dataset.title = state.title;
    }
  ...
  panel.postMessage = function (message, transfer) {
    return panel.whenReady().then(function () {
      ...
      if (!frameTarget) {
        return Promise.reject(new Error("No valid panel frame for " + panel.panelId + "."));
      }

      if (!frameTarget.isConnected) {
        return Promise.reject(new Error("Panel frame " + panel.panelId + " is not connected to the DOM."));
      }

      if (frameTarget && WeavyPostal) {
        return WeavyPostal.postToFrame(frameTarget.name, weavy.getId(), message, transfer);
      } else {
    ...
    var _removePanel = function (noHistory) {
      if (!panel.node.dataset.persistent || force) {
        if (panel.isOpen) {
          ...
          try {
            WeavyPostal.unregisterContentWindow(panel.frame.name, weavy.getId());
          } catch (e) {
  ...
  weavy.on(WeavyPostal, "request:reset", { weavyId: weavy.getId(), windowName: panel.frame.name }, function () {
    panel.reset();
  });

  // Close the panel from the inside
  weavy.on(WeavyPostal, "request:close", { weavyId: weavy.getId(), windowName: panel.frame.name }, function () {
    panel.close();
  });
